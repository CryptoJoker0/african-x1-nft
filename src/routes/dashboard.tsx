...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your AFRICAN X1 NFT dashboard: owned NFTs, mint history, transactions and wallet." },
    ],
  }),
...
function DashboardPage() {
  ...
  const { data: owned = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("nfts").select("*").eq("owner_user_id", user!.id).order("token_id");
      return data ?? [];
    },
  });
  ...
  if (loading || !user) {
    return <div className="mx-auto max-w-3xl px-4 py-20 text-center text-muted-foreground">Loading dashboard…</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
      <header className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-[0.3em] text-african-gold">Holder dashboard</div>
          <h1 className="mt-2 font-display text-3xl sm:text-4xl">Welcome back, <span className="text-gradient-cyber">{profile?.username || user.email?.split("@")[0]}</span></h1>
        </div>
        <div className="flex items-center gap-3">
          <WalletButton />
          <button
            onClick={async () => { await supabase.auth.signOut(); toast("Signed out"); navigate({ to: "/" }); }}
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Wallet" value={address ? `${address.slice(0, 6)}…${address.slice(-4)}` : "Not connected"} icon={<Wallet size={18} />} />
        <StatCard label="Owned NFTs" value={owned.length.toString()} icon={<ImageIcon size={18} />} accent />
        ...
        {([
          ...
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-sm font-medium transition ${
              tab === t.id ? "bg-cyber-cyan/15 text-cyber-cyan" : "text-muted-foreground hover:text-foreground"
            }`}
          >{t.label}</button>
        ))}
      </div>
        ...
        {tab === "nfts" && (
          owned.length === 0 ? (
            <EmptyPanel
              ...
              cta={<Link to="/mint" className="mt-3 inline-flex rounded-full bg-cyber-cyan px-4 py-2 text-sm font-semibold text-background hover:glow-blue">Mint your first</Link>}
            />
          ) : (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {owned.map((n: any) => (
                <div key={n.id} className="overflow-hidden rounded-2xl glass-card">
                  <img src={n.image_url || preReveal} alt={n.name} loading="lazy" className="aspect-square w-full object-cover" />
                  <div className="p-3">
            ...
            <div className="overflow-hidden rounded-2xl glass-card">
              <table className="w-full text-sm">
                <thead className="border-b border-white/5 text-left text-[10px] uppercase tracking-widest text-muted-foreground">
                  <tr>
                    <th className="p-4">Date</th><th className="p-4">Type</th><th className="p-4">Status</th>
                    <th className="p-4">Amount</th><th className="p-4">Signature</th>
                  </tr>
                </thead>
                <tbody>
                  {txs.map((t: any) => (
                    <tr key={t.id} className="border-b border-white/5 last:border-0">
                      <td className="p-4 text-muted-foreground">{new Date(t.created_at).toLocaleString()}</td>
                      <td className="p-4 capitalize">{t.tx_type}</td>
                      <td className="p-4"><StatusPill s={t.status} /></td>
                      <td className="p-4 text-african-gold">{t.amount ?? "—"}</td>
                      <td className="p-4 font-mono text-xs text-muted-foreground">{t.signature ? `${t.signature.slice(0,8)}…` : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function StatusPill({ s }: { s: string }) {
  const cls = s === "confirmed" ? "bg-cyber-cyan/15 text-cyber-cyan border-cyber-cyan/40" :
  ...
  return <span className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-widest ${cls}`}>{s}</span>;
}
...
function StatCard({ label, value, icon, accent }: { label: string; value: string; icon: React.ReactNode; accent?: boolean }) {
  return (
    <div className="glass-card rounded-2xl p-5">
      ...
      <div className={`mt-3 font-display text-2xl ${accent ? "text-african-gold" : ""}`}>{value}</div>
    </div>
  );
}
...
function ProfileForm({ userId, initial }: { userId: string; initial: any }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("profiles").upsert({
      id: userId, username, wallet_address: wallet || null,
    });
    setSaving(false);
  ...
  return (
    <form onSubmit={save} className="max-w-lg rounded-2xl glass-card p-6">
      <div className="mb-4 flex items-center gap-2 text-african-gold">
        <User size={16} /><span className="text-[10px] uppercase tracking-[0.25em]">Profile</span>
      </div>
      ...
      <button disabled={saving} className="rounded-full bg-cyber-cyan px-4 py-2 text-sm font-semibold text-background disabled:opacity-60">
